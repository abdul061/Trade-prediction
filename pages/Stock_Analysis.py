...
import datetime
import ta
from pages.utils.plotly_figure import candlestick, plotly_table, RSI, MACD, close_chart, Moving_average
...
if len(data) >= 2:
    ...
    col1.metric('Daily Change', str(round(data['Close'].iloc[-1], 2)), "N/A (Only 1 data point)")
else:
    col1.metric('Daily Change', "N/A", "No data")

if len(data) > 0:
    last_ten_df = data.tail(10).sort_index(ascending=False).round(2)
    fig_df = plotly_table(last_ten_df)
    st.write('##### Historical Data (Last 10 Days)')
    st.plotly_chart(fig_df, use_container_width=True)
else:
    st.warning("No historical stock data available for the selected date range.")
